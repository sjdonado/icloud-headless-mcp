...
| `internal/health` | the optional health extension: export importer, SQLite store, and six read-only tools. Only the importer opens the store read-write; the Drive layer never learns what it stages for it |
